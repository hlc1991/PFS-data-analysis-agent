"""Centralised logging configuration for PFS Data Analysis Agent.
...
File pattern:  pfs_YYYY-MM-DD.log   (one file per day, kept 30 days)
Active file is always named after today's date — no plain "pfs.log".
"""
import datetime
import logging
...
class _DailyFileHandler(logging.handlers.TimedRotatingFileHandler):
    """TimedRotatingFileHandler variant with daily file named pfs_YYYY-MM-DD.log"""

    def __init__(self, log_dir: Path, backup_count: int = 30, encoding: str = "utf-8"):
        self._log_dir = log_dir
        today = datetime.date.today().strftime("%Y-%m-%d")
        filename = str(log_dir / f"pfs_{today}.log")
        super().__init__(
            filename=filename,
            when="midnight",
            interval=1,
    ...
    def _namer(self, default_name: str) -> str:
        ...
        return str(self._log_dir / f"pfs_{date_str}.log")

    def doRollover(self):
        tomorrow = datetime.date.today().strftime("%Y-%m-%d")
        self.baseFilename = str(self._log_dir / f"pfs_{tomorrow}.log")
        super().doRollover()
