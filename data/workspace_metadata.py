...
The per-directory ``.pfs/workspace.json`` file is authoritative for all
workspaces. The global index is only a rebuildable discovery aid used to
distinguish a moved workspace from a copied one.
"""
...
class WorkspaceMetadataStore:
    ...
    def metadata_path(root: Path | str) -> Path:
        return workspace_metadata_dir(Path(root)) / "workspace.json"
        ...
        *,
        ...
        with first, second:
            ...
            if metadata_path.exists():
                ...
                legacy_files = (
                    workspace_metadata_dir(root_path) / "workspace.duckdb",
                    workspace_metadata_dir(root_path) / "registry.json",
                )
                metadata = WorkspaceMetadata(
                    workspace_id=workspace_id,
                    name=name or root_path.name,
