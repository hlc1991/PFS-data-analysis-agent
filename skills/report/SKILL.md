---
name: report
description: 规划结构化数据分析文档（analysis document）
icon: 📄
allowedTools: [get_schema, query_data, propose_report_outline]
---
# 分析文档规划

基于真实数据形成目标、受众、结论、证据和建议结构，引用关键指标与表。先提交分析文档大纲供确认，不直接生成最终文件；确认后由受控输出流程执行。
...
3. Use `propose_report_outline` to create the analysis-document structure. Do not export the final document in this skill turn.
4. After user confirmation, the controlled document command flow may call the export tool.
5. Keep the proposal compact: 4–6 sections and no more than 120 Chinese characters
   per section body. Submit an outline, not the complete document正文, so the tool-call
...
- Analysis-document implementation: `Function/Output/report_export.py`
