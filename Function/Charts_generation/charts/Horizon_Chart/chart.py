    ...
    **kwargs
    ...
    if is_wide:
        df = df.melt(
            ...
            var_name="_x_label",
            value_name="_horizon_value"
        ).rename(columns={id_col: "_series"})

        df["_horizon_value"] = pd.to_numeric(df["_horizon_value"], errors="coerce")
        ...
        df = df.dropna(subset=["_horizon_value"]).sort_values(["_series", "_x_label"])

        x_col = "_x_label"
        y_cols = ["_horizon_value"]
        series_col = "_series"
    ...
    if is_wide:
        long_df = df[[_x, "_series", "_horizon_value"]].rename(
            columns={"_horizon_value": "_value"}
        ).copy()
    else:
        long_df = None
