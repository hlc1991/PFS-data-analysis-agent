...
def _auto_order(series: pd.Series) -> Tuple[int, int, int]:
    ...
    try:
        ...
        # 某些趋势很强、近似常量或病态的序列会让 auto_arima 的内部
        # 单位根检验/候选模型搜索失败。保留原错误边界，但在有限候选
        # 阶数中寻找一个能真实拟合的模型，避免自动模式被单一候选拖垮。
        fallback = _fallback_order(series)
        if fallback is None:
            raise RuntimeError(
                "ARIMA 自动选阶失败，且有限候选阶数均无法拟合；"
                "请检查数据是否包含足够的非空数值或改用手动阶数。"
            ) from exc
        return fallback


def _fallback_order(series: pd.Series) -> Optional[Tuple[int, int, int]]:
    """在 auto_arima 数值失败时，用有限候选模型按 AIC 选择可拟合阶数。"""
    from statsmodels.tsa.arima.model import ARIMA as _ARIMA

    stationary, _ = _adf_test(series)
    d = 0 if stationary else 1
    candidates = [(0, d, 0), (1, d, 0), (0, d, 1), (1, d, 1), (2, d, 0), (0, d, 2)]
    fitted = []
    for order in candidates:
        try:
            result = _ARIMA(series, order=order).fit(method_kwargs={"warn_convergence": False})
            if np.isfinite(result.aic):
                fitted.append((float(result.aic), order))
...
def run(
    ...
    try:
        ...
        if manual_order is not None:
            raise
        fallback = _fallback_order(series)
        if fallback is None or fallback == order:
            raise RuntimeError(
                f"ARIMA 阶数 {order} 拟合失败，且有限候选阶数均无法拟合；"
                "请检查数据或改用手动阶数。"
            ) from exc
        order = fallback
        progress(50, f"自动选阶模型拟合失败，已回退到 {order}")
        fitted, forecast_df, residuals, model_result = _fit_predict(series, order, steps)
